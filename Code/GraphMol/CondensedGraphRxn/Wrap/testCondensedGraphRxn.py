...
from rdkit.Chem.CondensedGraphRxn import rdCondensedGraphRxn as CRS
...
class TestCondensedGraphRxn(unittest.TestCase):
  ...
  def test1Basicsreader(self):
    i=0
    for s,c in zip(self.smarts, self.crss):
        self.assertEqual( s , CRS.CRSreader(c))
        i+=1

  def test2Basicswriter(self):
    i=0
    for s,c in zip(self.smarts, self.crss):
        self.assertEqual( c , CRS.CRSwriter(s))
        i+=1
  ...
  def test3Acetal(self):
    ...
    crs = 'CC1(C)({=!}O){!-}OCCO{!-}1' 
    print('test3Acetal writer:',CRS.CRSwriter(sma))
    self.assertEqual (crs, CRS.CRSwriter(sma))
    print('test3Acetal reader:',CRS.CRSreader(crs))
  ...
  def testCanParse(self):
    crs = 'C{!-}OC(=O)CC1=C({-!}Br)C=C(F)C=C1'
    ...
    print('testCanParse:',CRS.CRSreader(crs))
    self.assertEqual (sma, CRS.CRSreader(crs))
  ...
  def testSignature(self):
    ...
    crs1 = 'C{!-}CO'
    crs2 = 'COC{!-}C'
    self.assertEqual (crs0, CRS.CRSwriter(sma, signature=True, radius = 0))
    self.assertEqual(crs1, CRS.CRSwriter(sma, signature=True, radius = 1))
    self.assertEqual(crs2, CRS.CRSwriter(sma, signature=True, radius = 2))
  ...
  def testAldolCondensation(self):
      ...
      crs = "CC(C=O){!=}C({=!}O)c1ccc(C(C)C)cc1"
      crs0 = "C{!=}C{=!}O"
      crs1 = "CC(C){!=}C(c){=!}O"
      crs2 = "cc(c)C({=!}O){!=}C(C)C=O"
      crs3 = "ccc(cc)C({=!}O){!=}C(C)C=O"
      sign = [crs0,crs1,crs2,crs3]

      # Test the full molecule
      self.assertEqual(crs,CRS.CRSwriter(sma))
      for r,expt in zip(list(range(4)),sign):
          self.assertEqual(expt,CRS.CRSwriter(sma,signature=True,radius=r))
  ...
  def testCase1(self,display=False):
      ...
      self.assertEqual( CRS.RxnCompleteMapping(sma, debug=display), expt)
  ...
  def testCase16(self,display=False):
      ...
      print('16:', CRS.RxnCompleteMapping(sma))
