...
struct bondid {
  ...
  bondid(unsigned int fromnum, unsigned int tonum, unsigned int fromidx,
         unsigned int toidx, std::string bondtype) {
  ...
  bool operator==(const bondid &other) {
...
struct NbrType {
  ...
  // comparation equal
  bool operator==(const NbrType &other) {
...
struct AtomType {
  ...
  AtomType(unsigned int m, unsigned int i, unsigned int a, std::size_t myinv,
           ROMOL_SPTR mol, std::vector<uint16_t> nei, bool r) {
    inv = myinv;
    mnum = m;
    atno = a;
    idx = i;
  ...
  // comparation atomtype smaller
  bool operator<(const AtomType &other) const { return idx < other.idx; }
};
...
struct mapBond {
  ...
  bool operator==(const mapBond &other) {
...
struct matchPair {
  ...
  bool operator<(const matchPair &other) const { return t > other.t; }
};

void clearAtomMap(std::vector<ROMOL_SPTR> &mollist) {
  // unsigned int sanitizeOps = MolOps::SANITIZE_ALL ^ MolOps::SANITIZE_KEKULIZE
...
std::string convertMolstoSmiles(const std::vector<ROMOL_SPTR> &listofmol,
                                      bool debug = false) {
  std::stringstream ss;
  int f = 0;
  if (debug) {
...
// convert to string the results
std::string VectorSmilestoSmiles(std::vector<std::string> smis,
                                       bool debug = false) {
  std::stringstream ss;
  int f = 0;
  if (debug) {
...
// check if an mapped fragment is full enclosed (part) of another mapped
// molecule
bool isEnclosed(ROMOL_SPTR mol, std::vector<ROMOL_SPTR> mollist, bool debug) {
  ...
  // TO DO : must check that mol is a combination of multiple reagant and not
  // only one like here! possible solution use a while to expend the list of mol
  // combinatorial n-1 join ! or use not Enclose solution mean
...
bool isCovered(ROMOL_SPTR mol, std::vector<unsigned int> dstlist, bool debug) {
  ...
  std::set_difference(molset.begin(), molset.end(), dstlist.begin(),
                      dstlist.end(), std::inserter(diff, diff.begin()));
  if (debug) {
...
void removeMonoAtomreagents(std::vector<ROMOL_SPTR> &molR,
                            std::vector<ROMOL_SPTR> &molP) {
...
void finalCleanUp(std::vector<ROMOL_SPTR> mollist,
                  std::vector<ROMOL_SPTR> &reslist, bool debug = false) {
  if (debug) {
  ...
  for (it1 = mollist.begin(); it1 != mollist.end(); ++it1) {
    ...
    for (it2 = mollist.begin(); it2 != mollist.end(); ++it2) {
      if (it2 < it1) {
        ...
        } else if (std::includes(set2.begin(), set2.end(), set1.begin(),
                                 set1.end())) {
...
void getBondInventory(std::vector<ROMOL_SPTR> mollist,
                      std::vector<bondid> &full) {
  std::vector<ROMOL_SPTR>::iterator it;
  for (it = mollist.begin(); it != mollist.end(); ++it) {
...
unsigned int getNumHs(Bond::BondType BT) {
  unsigned int addH;
  switch (BT) {
...
bool isFullyMapped(ROMOL_SPTR mol) {
  bool res = true;
  for (auto atom : mol->atoms()) {
    if (res) {
...
bool isValidMapping(ROMOL_SPTR mol, std::vector<mapBond> btab) {
  bool valid = true;
  ...
  for (it = diff.begin(); it != diff.end(); ++it) {
    ...
    for (cit = changes.begin(); cit != changes.end(); ++cit) {
      ...
      for (uint16_t nidx : nbrlist) {
        ...
        if (!bond) {
          // std::cout << "No bond between " << natidx << " and " << atidx  <<
          // std::endl;
        } else {
          if (atomnum > 0 && natomnum > 0) {
            bondid query(atomnum, natomnum, atidx, natidx,
                         bond->getBondTypeWithAtoms(false));
            if (std::find(conserved.begin(), conserved.end(), query) !=
                conserved.end()) {
              if (debug) {
                std::cout << "Conserved: " << atomnum << ", " << natomnum
                          << std::endl;
              }
              continue;
            }
    ...
    std::vector<ROMOL_SPTR> listofmol = MolOps::getMolFrags(
        *cpMol, false);  // in Ruud python code value is set to true ???
    if (debug) {
    ...
    for (ROMOL_SPTR fragmol : listofmol) {
      ...
      if (fragmol && isAllAtomsValid(fragmol) &&
          fragmol->getNumHeavyAtoms() > 0) {
        // try {
        if (debug) {
        ...
        MolOps::SANITIZE_KEKULIZE ^ MolOps::SANITIZE_PROPERTIES;
        ...
        try {
            ...
            BOOST_LOG(rdInfoLog) << "Issue to sanitize fragmol after
        removeHs\n";
        }
        */
...
void getComputeSolutions(ROMOL_SPTR mol1, ROMOL_SPTR mol2,
                         std::vector<mapBond> btab,
                         std::vector<RDKit::MatchVectType> matches,
                         std::vector<matchPair> &valid_hit, bool debug) {
  // bool found = false;
  for (size_t i = 0; i < matches.size(); ++i) {
    ...
    if (debug) {
      ...
      std::cout << "--------------------------------------" << std::endl;
      std::cout << "transferred: " << transferred << "/" << matches[i].size()
                << std::endl;
...
void transferMapping(std::vector<mapBond> btab,
                     std::vector<ROMOL_SPTR> &mollist, bool debug) {
  // need to sort first largest to smallest molecule
...
void getActiveReagants(std::vector<ROMOL_SPTR> allmolR,
                       std::vector<ROMOL_SPTR> allmolP,
                       std::vector<ROMOL_SPTR> &goodmolR) {
  ...
  for (it = allmolR.begin(); it != allmolR.end(); ++it) {
    ...
    for (auto atom : (*it)->atoms()) {
      ...
      if (!keep && atnum > 0) {
        keep = std::find(mapatomsP.begin(), mapatomsP.end(), atnum) !=
               mapatomsP.end();
      }
    }
    if (keep) {
...
void getAcceptMols(std::vector<ROMOL_SPTR> allmol,
                   std::vector<ROMOL_SPTR> &goodmol) {
  std::vector<ROMOL_SPTR>::iterator it;
...
void addLG(std::vector<bondid> conserved, std::vector<ROMOL_SPTR> rst,
           std::vector<ROMOL_SPTR> &dst, bool debug) {
  if (conserved.size() > 0) {
    ...
    // loop over reagants and check if any atommapnum is not present in product
    // list
    ...
    for (rit = rst.begin(); rit != rst.end(); ++rit) {
      ...
      // if changed done please add new Fragments to product molecules list if
      // Enclosed is valid
      if (changed) {
        // add new mols to dist list
        if (debug) {
...
std::string RXNCompleteMapping(std::string sma, bool debug,
                               bool addleavinggroups) {
  ...
  // use useSmile = true and aromatize/sanitize = true (parameter expose in
  // python!)
  v2::ReactionParser::ReactionSmartsParserParams params;
  params.sanitize = true;
  ChemicalReaction *rxn =
      v2::ReactionParser::ReactionFromSmiles(sma, params).release();
  if (debug) {
    // check data process
  ...
  // extra steps in c++ to keep only active reagants and remove monoAtoms
  // reagants
  ...
  if (debug) {
    ...
    std::cout << convertMolstoSmiles(rkeep, false) << ">>"
              << convertMolstoSmiles(pkeep, false) << std::endl;
    std::cout << "-----------------" << std::endl;
  }
  ...
  // we don't nned to do the clean up on reagant cause it may be partially
  // unmapped
  ...
  if (debug) {
    ...
    std::string rs1 =
        convertMolstoSmiles(rclean, false);  // replace rclean by rkeep
  ...
  getAtomInventory(pkeep, patoms, productnondupatnum, false);
  getAtomInventory(rclean, ratoms, reactantnondupatnum,
                   true);  // replace rclean by rkeep
  ...
  if (debug) {
    ...
    std::string rs2 =
        convertMolstoSmiles(rclean, false);  // relace rclean by rkeep
  ...
  std::string rs =
      convertMolstoSmiles(rclean, false);  // replacee rclean by rkeep
  sma = rs + ">>" + ps;
  if (debug) {
    // check data process
...
bool getRXNCompTotal(std::string rxnsma, std::string rxncoresma) {
  ...
  std::unique_ptr<ChemicalReaction> core(
      RxnSmartsToChemicalReaction(rxncoresma));
      ...
      for (auto at : (*prod).atoms()) {
        if (at->hasProp(common_properties::reactionMapNum)) {
          ...
          if (mapnum > 0) {
            ...
            unsigned int reactIdx =
                at->getProp<unsigned int>(common_properties::reactantAtomIdx);

            reactants[reactMatToReactant[mapnum]]
                ->getAtomWithIdx(reactIdx)
                ->setAtomMapNum(mapnum);
          }
        }
      }
